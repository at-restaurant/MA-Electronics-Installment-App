// src/components/CustomerCard.tsx - FULLY MOBILE RESPONSIVE
...
export default function CustomerCard({
    ...
    // ✅ COMPACT VARIANT - For Collection Page
    if (variant === "compact") {
        return (
            <div
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        {/* ✅ TRUNCATE NAME */}
                        <h3 className="font-semibold text-sm truncate">{truncateName(customer.name)}</h3>
                        {/* ✅ COMPACT CURRENCY */}
                        <p className="text-xs text-gray-500 truncate">{formatCurrencyCompact(customer.installmentAmount)}</p>
                    </div>
                    <span className="text-xs text-gray-400 flex-shrink-0">{daysOverdue}d</span>
                </div>
            </div>
        );
    }

    // ✅ DAILY VARIANT - Collection page with checkbox style
    if (variant === "daily") {
        return (
            <div
                onClick={() => onClick(customer)}
                className="bg-white rounded-xl p-3 shadow-sm cursor-pointer transition-all active:scale-[0.98] hover:shadow-md"
            >
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                                ...
                                <span>•</span>
                                <span className="truncate">{formatCurrencyCompact(remaining)} left</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border-2 border-transparent hover:border-blue-200 transition-all">
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                            ...
                            <span className={`px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap flex-shrink-0 ${getRiskColor(riskLevel)}`}>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3 pb-3 border-b border-gray-100">
                    <div>
                        ...
                        {/* ✅ COMPACT + TRUNCATE */}
                        <p className="font-bold text-orange-600 text-xs truncate">{formatCurrencyCompact(remaining)}</p>
                    </div>
                    <div>
                        <p className="text-xs text-gray-500">Last</p>
                        <p className="font-medium text-xs">{daysOverdue}d ago</p>
                    </div>
                    <div>
                        <p className="text-xs text-gray-500">Done</p>
                        <p className="font-bold text-xs">{progress}%</p>
                    </div>
                </div>
                ...
                {showActions && onActionClick && (
                    <div className="grid grid-cols-2 gap-2">
                        <button
                            ...
                            className="py-2 px-3 bg-blue-50 text-blue-600 rounded-lg font-medium text-xs hover:bg-blue-100 transition-colors"
                        >
                            View
                        </button>
                        <button
                            ...
                            className="py-2 px-3 bg-green-50 text-green-600 rounded-lg font-medium text-xs hover:bg-green-100 transition-colors"
                        >
                            Alert
                        </button>
                    </div>
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold shadow-md overflow-hidden flex-shrink-0">
                        {customer.photo ? (
                            <Image
                                src={customer.photo}
                                alt={customer.name}
                    ...
                    <div className="flex-1 min-w-0">
                        {/* ✅ TRUNCATE NAME */}
                        <h3 className="font-semibold text-gray-900 text-sm truncate">{truncateName(customer.name)}</h3>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ${getStatusColor(isCompleted, daysOverdue)}`}>
                ...
                <div className="flex justify-between text-xs pt-2">
                    <span className="text-gray-600 truncate">
                        ...
                        <span className="font-semibold text-green-600">
                            {formatCurrencyCompact(customer.paidAmount)}
                        </span>
                    </span>
