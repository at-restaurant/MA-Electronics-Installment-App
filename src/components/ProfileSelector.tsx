...
export default function ProfileSelector({ onSelect }: { onSelect: (profile: any) => void }) {
    ...
    return (
        <div className="fixed inset-0 bg-gradient-to-br from-blue-600 via-purple-600 to-pink-500 z-50 flex flex-col">
                ...
                >
                    {profiles.map((profile, index) => (
                        ...
                        >
                            ...
                            >
                                <div className={`w-24 h-24 rounded-full bg-gradient-to-br ${profile.gradient} mb-6 flex items-center justify-center shadow-lg`}>
                                    <Users className="w-12 h-12 text-white" />
                                </div>
                                <h2 className="text-3xl font-bold text-center mb-3">{profile.name}</h2>
                                <p className="text-gray-500 text-center text-sm mb-4">{profile.description}</p>
            ...
            <div className="pb-8 text-center text-white">
                <p className="text-sm mb-3 opacity-90">👆 Swipe up/down to switch profiles</p>
