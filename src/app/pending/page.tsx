...
import { AlertCircle, Clock, Phone, MessageSquare, ChevronRight } from 'lucide-react';
...
export default function PendingPage() {
    ...
    const loadPendingCustomers = (profileId: number) => {
        ...
        const pending = allCustomers
            .filter((c: any) => c.profileId === profileId && c.paidAmount < c.totalAmount)
            .map((c: any) => ({
                ...c,
                ...
                <div className="space-y-3">
                    {filteredCustomers.length === 0 ? (
                        ...
                        filteredCustomers.map(customer => {
                          ...
                          {isOverdue ? '⚠ Overdue' : riskLevel === 'medium' ? '⚡ Warning' : 'Active'}
                        </span>
                                            </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mb-3 pb-3 border-b border-gray-100">
                                        <div>
                                            ...
                                            <p className="font-bold text-orange-600">{formatCurrency(customer.remaining)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Last Payment</p>
                                            <p className="font-medium text-sm">{formatDate(customer.lastPayment)}</p>
                                        </div>
                                        <div>
                ...
                {filteredCustomers.length > 0 && (
                    <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                        <p className="text-sm text-amber-800">
                            <strong>💡 Quick Tip:</strong> Send reminders via WhatsApp to customers who are overdue.
                            Regular communication helps maintain good payment discipline.
                        </p>
                    </div>
                )}
            </div>
